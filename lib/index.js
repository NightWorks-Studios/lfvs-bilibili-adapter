...
__name(randomSample, "randomSample");
var generateDmParams = /* @__PURE__ */ __name(() => {
...
var magicStr = "FcwAPNKTMug3GV5Lj7EJnHpWsx4tb8haYeviqBz6rkCy12mUSDQX9RdoZf";
var MAX = 1n << 51n;
var table = {};
...
var LISFOX_PROXY_VIEW_API = "http://server.lisfox.top:9003/api/proxy/view";
var NAV_API = "https://api.bilibili.com/x/web-interface/nav";
var QRCODE_GEN_API = "https://passport.bilibili.com/x/passport-login/web/qrcode/generate";
var QRCODE_POLL_API = "https://passport.bilibili.com/x/passport-login/web/qrcode/poll";
var BilibiliAdapterService = class extends Service {
  constructor(ctx, config) {
    ...
    ctx.inject(["webui"], (ctx2) => {
      ctx2.webui.addEntry({
        ...
        "bilibili/status": /* @__PURE__ */ __name(() => this.getStatus(), "bilibili/status")
      });
    });
  ...
  config;
  static {
    __name(this, "BilibiliAdapterService");
  }
  static inject = {
    http: { required: true },
    timer: { required: true },
  ...
  getStatus() {
    const now = Date.now();
    this.rpmLisfoxTimestamps = this.rpmLisfoxTimestamps.filter((t) => now - t < 6e4);
    this.rpmLocalTimestamps = this.rpmLocalTimestamps.filter((t) => now - t < 6e4);
  ...
  async start() {
    ...
    if (this.loadCookie()) {
      const navData2 = await this.ctx.http.get(NAV_API, { headers: { Cookie: this.cookie } });
      if (navData2.code === 0 && navData2.data.isLogin) {
        this.setStatus("logged_in", { mid: navData2.data.mid, uname: navData2.data.uname });
        this.setOnline();
        return;
      }
    }
  ...
  async saveCookie(headers) {
    ...
    if (typeof headers.get === "function") {
      if (typeof headers.getSetCookie === "function") {
        ...
        setCookie = val ? val.split(",") : void 0;
      }
    } else {
    ...
    this.cookie = cookiesArray.map((c) => c.split(";")[0]).join("; ");
  ...
  async getVideoInfoAndStats(videoId) {
    ...
    if (this.config.useLisfoxProxy) {
      try {
        ...
        const lisfox = await this.ctx.http.post(LISFOX_PROXY_VIEW_API, { bvid: videoId }, { timeout: 5e3 });
        if (lisfox?.response) {
  ...
  mapBilibiliViewData(data) {
    ...
    return {
      info: {
        ...
        pubdate: new Date((data.pubdate || arc.pubdate) * 1e3),
  ...
  async getUploaderRecentVideos(mid) {
    const start = Date.now();
    try {
      const params = {
        mid,
      ...
      if (res.code === 0 && res.data?.list?.vlist) {
        const videos = res.data.list.vlist.map((v) => ({
          platform: "bilibili",
          videoId: v.bvid,
          title: v.title,
          pic: v.pic,
          pubdate: new Date(v.created * 1e3),
  ...
  async getUploaderInfo(mid) {
    const start = Date.now();
    try {
      const params = { mid, wts: Math.round(Date.now() / 1e3) };
      const query = await this.wbiSign(params);
